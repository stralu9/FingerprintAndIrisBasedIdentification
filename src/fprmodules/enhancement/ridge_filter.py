...
def ridge_filter(im, orient, freq, kx, ky):
    ...
    x,y = np.meshgrid(np.linspace(-sze,sze,(2*int(sze) + 1)),np.linspace(-sze,sze,(2*int(sze) + 1)));
    ...
    newim = np.zeros((gabor_filter.shape[0],rows,cols));
    ...
    maxorientindex = np.round(180/angleInc);                       #60
    orientindex = np.round(orient/np.pi*180/angleInc);             #lungo quanto orient
    ...
    for theta in range(gabor_filter.shape[0]):
        for k in range(0,finalind_cols):
            ...
            newim[theta][r][c] = np.sum(img_block * gabor_filter[theta]);
...
#         newim[0][r][c] = np.sum(img_block * gabor_filter[int(orientindex[r][c]) - 1])
            
    final_im = np.zeros((newim.shape[1], newim.shape[2]))
    for r in range(newim.shape[1]):
        for c in range(newim.shape[2]):
            final_im[r][c] = newim[:,r,c].max()
