...
def winStd(img, wlen):
    wmean, wsqrmean = (cv2.boxFilter(x, -1, (wlen, wlen), normalize = True, borderType=cv2.BORDER_REFLECT) for x in (img, img*img))
    return np.sqrt(np.abs(wsqrmean - wmean*wmean))

def largest_connected_component(maskin):
	maskin = maskin.astype('uint8')
	nb_components, output, stats, centroids = cv2.connectedComponentsWithStats(maskin,4)
